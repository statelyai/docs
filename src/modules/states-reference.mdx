...
`state.children` is an object mapping spawned actor/actor IDs to their instances. See [📖 Referencing Actors](./communication.md#referencing-actors) for more details.
...
const actor = invoke(machine)
  .onTransition((state) => {
    state.children.notifier; // actor from createNotifier()
    state.children.logger; // actor from createLogger()
  })
  .start();
```
...
You can then interpret the machine from this state by passing the `State` into the `.start(...)` method of the interpreted actor:

```js
// ...

// This will start the actor at the specified State
const actor = interpret(myMachine).start(previousState);
```
...
- You should never have to create a `State` instance manually. Treat `State` as a read-only object that _only_ comes from `machine.transition(...)` or `actor.onTransition(...)`.
