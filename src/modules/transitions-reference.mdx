...
const gameActor = interpret(gameMachine)
...
Note that when defining multiple transitions with the same event name in a child ancestor-descendant chain, the most inner transition will exclusively be taken. In the example above, this is why the `logTelemetry` action defined in the parent `LOG` event won’t execute as soon as the machine reaches the `userInfoPage` state.
