...
The behavior and output of actors can be tested by asserting that it _eventually_ reaches an expected state, given an initial state and a sequence of events:

```js
import { fetchMachine } from '../path/to/fetchMachine';

it('should eventually reach "success"', (done) => {
  const fetchActor = interpret(fetchMachine).onTransition((state) => {
  ...
  // send zero or more events to the actor that should
  // cause it to eventually reach its expected state
  fetchActor.send({ type: 'FETCH', id: 42 });
});
```
...
it('should eventually reach "success"', (done) => {
  ...
  const fetchActor = interpret(mockFetchMachine).onTransition((state) => {
    if (state.matches('success')) {
