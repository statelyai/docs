...
The `@xstate/react` package also provides a `createActorContext(…)` function for convenience. Read the blog post about [making state machines global in React](2023-1-27-making-state-machines-global-in-react) and see the [documentation on `createActorContext(…)`](/docs/xstate-react#createactorcontextlogic).
