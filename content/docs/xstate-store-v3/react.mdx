...
These are the docs for `@xstate/store-react` v1. For the latest version, see the [latest `@xstate/store-react` docs](/docs/xstate-store/react).
</Callout>

The `@xstate/store-react` package provides React bindings for [XState Store](/docs/xstate-store-v3). It includes hooks for subscribing to store state and creating local component stores.
...
```tsx
import { useStore, useSelector } from '@xstate/store-react'; // [!code highlight:1]
...
```tsx
import { createAtom } from '@xstate/store-react'; // [!code highlight:1]
