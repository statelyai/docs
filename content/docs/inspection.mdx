...
**Observing transient states:** Subscribers to an actor only receive the *final* snapshot after all transitions (including eventless ones) are applied. So states that are only entered and left via `always` transitions in the same step never appear in `subscribe()`, `waitFor()`, or when using `snapshot.matches()` / `snapshot.hasTag()`. To observe those intermediate states, use the `@xstate.microstep` inspection event. For more detail and alternatives (such as `after: { 0: '...' }` when you need observability), see [Eventless (always) transitions – Observability and transient states](eventless-transitions#observability-and-transient-states).
...
<details>
<summary>Example of microstep events</summary>
