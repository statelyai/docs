...
<Callout>
  The `state.can(...)` method will also check transition guards by executing
  them. Transition guards should be pure functions.
</Callout>
...
## `mapState(snapshot, mapper)` [#map-state]

_Since XState version 5.31.0_

The `mapState(snapshot, mapper)` function maps a machine snapshot's active state nodes to values using a hierarchical mapper object. It traverses all active state nodes (from leaf to root) and collects results from matching `map` functions.

```ts
import { createActor, mapState, setup } from 'xstate';
...
// [!code highlight:12]
const results = mapState(actor.getSnapshot(), {
  map: ({ context }) => `root:${context.count}`,
  states: {
    active: {
      map: ({ context }) => `active:${context.count}`,
      states: {
        idle: {
          map: ({ context }) => `idle:${context.count}`,
        },
      },
    },
  },
...
// ['idle:42', 'active:42', 'root:42']
```

The mapper mirrors the state hierarchy of the machine. The top-level `map` applies to the root state node, and nested `states` match child state nodes by key. Only active states are mapped; inactive states are skipped.

Each result includes a `result` (the return value) and a `stateNode` (the matched state node).

### Parallel states

All active regions and their active children are mapped:
...
const results = mapState(createActor(machine).getSnapshot(), {
  map: () => 'root',
  states: {
    region1: {
      map: () => 'region1',
...
console.log(results.map((r) => r.result));
// ['x', 'region1', 'p', 'region2', 'root']
```

### Partial mapping

You don't need to provide a `map` function at every level. Map only the states you care about:
...
`mapState` is fully typed: state keys are validated against the machine's state hierarchy, and the snapshot type is inferred in `map` functions.
