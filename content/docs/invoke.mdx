...
You can invoke [event observables](./observable-actors) with the `fromEventObservable(...)` actor logic creator. Event observable logic is similar to observable logic in that the parent actor subscribes to the event observable; however, emitted values are events sent directly to the invoking parent actor.
