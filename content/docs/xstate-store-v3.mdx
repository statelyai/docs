...
description: Simple state management for JavaScript/TypeScript apps in @xstate/store v3.
---
...
<Callout>
These are the docs for version 3.x of XState Store. For the latest version, see [XState Store docs](./xstate-store). For version 2.x, see [Version 2.x docs](./xstate-store-v2).
</Callout>
...
- **React**: [`@xstate/store-react`](/docs/xstate-store-v3/react)
- **Vue**: [`@xstate/store-vue`](/docs/xstate-store-v3/vue)
- **Svelte**: [`@xstate/store-svelte`](/docs/xstate-store-v3/svelte)
- **Solid**: [`@xstate/store-solid`](/docs/xstate-store-v3/solid)
- **Angular**: [`@xstate/store-angular`](/docs/xstate-store-v3/angular)
- **Preact**: [`@xstate/store-preact`](/docs/xstate-store-v3/preact)
...
Use the `useSelector(store, selector)` hook from [`@xstate/store-react`](/docs/xstate-store-v3/react) to subscribe to the store and get the current state.
...
For the full React API including `useStore`, `useAtom`, and `createStoreHook`, see the [React documentation](/docs/xstate-store-v3/react).

## Usage with other frameworks

- **Vue**: See [`@xstate/store-vue`](/docs/xstate-store-v3/vue)
- **Svelte**: See [`@xstate/store-svelte`](/docs/xstate-store-v3/svelte)
- **Solid**: See [`@xstate/store-solid`](/docs/xstate-store-v3/solid)
- **Angular**: See [`@xstate/store-angular`](/docs/xstate-store-v3/angular)
- **Preact**: See [`@xstate/store-preact`](/docs/xstate-store-v3/preact)
...
The `persist` extension persists store context to storage (such as `localStorage`, `sessionStorage`, or async adapters like React Native's `AsyncStorage`). It automatically saves state after events and restores it when the store is created.

### Basic Usage

Import `persist` from `@xstate/store/persist` and use it with the `.with()` method:
...
The store context will be saved to `localStorage` (default) on every event and automatically restored when the store is created.

### Storage Adapters

By default, `persist` uses `localStorage`. You can provide a custom storage adapter that implements `getItem`, `setItem`, and `removeItem`:
...
For SSR environments, use `createJSONStorage()` which provides a safe fallback when storage is unavailable:

```ts
import { persist, createJSONStorage } from '@xstate/store/persist';
...
Use `pick` to only persist specific fields from the context:

```ts
const store = createStore({
  context: { count: 0, tempData: 'not persisted' },
...
Use `filter` to prevent certain events from triggering persistence:
...
const store = createStore({
  ...
  on: {
    inc: (ctx) => ({ count: ctx.count + 1 }),
    tempChange: (ctx) => ({ count: ctx.count }),
  },
}).with(
  persist({
    name: 'my-store',
    filter: (event) => event.type !== 'tempChange',
  })
);
```
...
Use `version` and `migrate` for schema management when your context shape changes:
...
Use `throttle` to limit how frequently data is written to storage:
...
Use `flushStorage(store)` to force an immediate write of any pending throttled data:
...
Use `serialize` and `deserialize` for custom encoding/decoding:

```ts
const store = createStore({
  context: { date: new Date() },
  on: {
    setDate: (ctx, e: { date: Date }) => ({ date: e.date }),
  },
}).with(
  persist({
    name: 'my-store',
    serialize: (ctx) => JSON.stringify({
...
By default, the store is hydrated automatically on creation. To control when hydration happens, use `skipHydration` and `rehydrateStore()`:

```ts
import { persist, rehydrateStore, isHydrated } from '@xstate/store/persist';
...
isHydrated(store); // false

// Manually rehydrate later (async)
await rehydrateStore(store);

isHydrated(store); // true
```

### Event Strategy

By default, `persist` uses the `'snapshot'` strategy which saves the context directly. You can use the `'event'` strategy to persist the event log instead and replay events on rehydration:
...
const store = createStore({
  ...
  persist({
    name: 'my-store',
    strategy: 'event',
    maxEvents: 100, // checkpoint after 100 events
  })
);
```

With the event strategy:
- Events are persisted instead of context snapshots
- On rehydration, events are replayed to reconstruct state
- `maxEvents` creates automatic checkpoints to limit replay time and storage size

### Clearing Persisted Data

Use `clearStorage()` to remove persisted data:
...
Use `onDone` and `onError` to handle persistence success and failure:
...
const store = createStore({
  ...
  persist({
    name: 'my-store',
    onDone: () => console.log('Persisted successfully'),
...
The `reset` extension allows you to reset the store context back to its initial state. This is useful for clearing form data, resetting user sessions, or returning to a known state.

### Basic Usage

Import `reset` from `@xstate/store/reset` and use it with the `.with()` method. The store will have a `reset()` method available on `store.trigger`:
...
You can customize what the store resets to using the `to` option. This function receives the initial `context` and current `context`, letting you preserve specific fields:
...
store.trigger.reset();
store.getSnapshot().context;
// => { count: 0, user: 'Alice' } — count reset, user preserved
```
