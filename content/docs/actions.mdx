...
Prefer `sendTo(...)`: pass actor refs (such as the parent actor ref) to other actors via [input](./input) or events, then store them in `context`. This avoids tight coupling and can be more type-safe than `sendParent(...)`.
