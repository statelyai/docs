...
- **`waitFor(actor, predicate)`** will never see those intermediate states, so a predicate that matches a transient state or its [tags](tags) will not resolve.
- **`snapshot.matches(...)`** and **`snapshot.hasTag(...)`** will never be true for a state that is only reached and left via `always` in one step.

Entry and exit [actions](actions) on those states still run; only the snapshot emission is skipped.

**When you need to observe or wait for such a state** (for example in tests or when matching tags):

1. **Use the [inspection API](inspection)** and listen for `@xstate.microstep` events. Each microstep (including eventless transitions) is reported there, so you can detect when the machine passes through a transient state.
