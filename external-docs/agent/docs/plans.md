...
The `allowedEvents` list accepts the same exact-type and wildcard forms as `agent.decide`; see [`allowedEvents` patterns](/docs/packages/agent/decisions#allowedevents-patterns).
...
- **Survives persist/resume.** `runAgent` holds the in-progress plan as the invoke child's own snapshot `context` (`{ applied, stepsRemaining, stopped }`), so a persisted snapshot captures it at `children.<id>.snapshot.context` and a plan can resume mid-run. On the [step path](/docs/packages/agent/steps#plans) the same plan re-surfaces as a `kind: 'plan'` effect each frontier: the host resolves one decision per frontier and derives the applied trail from the journal (see [Plans](/docs/packages/agent/steps#plans)).
...
- [The step path](/docs/packages/agent/steps#plans): plans as re-surfacing `kind: 'plan'` effects with crash-safe mid-plan resume.
