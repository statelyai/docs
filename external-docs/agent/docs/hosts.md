...
[`getAgentEffects`](/docs/packages/agent/steps) surfaces a decision effect whose request's `events` field holds only the events legal from the current snapshot ([`allowedEvents`](/docs/packages/agent/decisions) intersected with XState guards). Resolve it to the chosen, validated event with `resolveDecision`:
