...
This is different from XState `meta`, which describes state nodes/transitions for tooling (see [`../examples/email-drafter/index.ts`](https://github.com/statelyai/agent/blob/main/examples/email-drafter/index.ts) for a schema-typed `meta` example). Text/decision logic `metadata` is runtime input passed to the host executor.
