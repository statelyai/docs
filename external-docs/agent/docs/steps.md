...
For a live, in-process run, use [`runAgent`](/docs/packages/agent/hosts) instead: it owns an actor and settles `done | idle | error`. The step path is the same machine driven by hand, so a crashed process resumes without re-billing model calls.
...
Taught from [examples/ai-sdk-game-host/index.ts](https://github.com/statelyai/agent/blob/main/examples/ai-sdk-game-host/index.ts), the canonical thin loop. One host-owned primitive resolves a single frontier effect into the event to journal (or `undefined` for a fire-and-forget action):
...
For the durable, resume-by-replay flavor of this same loop (persist nothing but the journal, rebuild the frontier with `replay` each turn), see [examples/cloudflare-workers-ai-host/index.ts](https://github.com/statelyai/agent/blob/main/examples/cloudflare-workers-ai-host/index.ts).
...
**Compaction.** A snapshot taken mid-flight cannot carry in-flight effect state; the journal can. So snapshot only at **quiescent / idle** points. Resume from an idle snapshot plus the entries appended since, or replay the whole journal from index 0. Both yield the identical state. See [Checkpoints and the event log](/docs/packages/agent/checkpoints) for the `AgentEventLogStore` protocol and the snapshot-as-compaction cache.

## Plans

An [`agent.plan`](/docs/packages/agent/plans) invoke applies an ordered sequence of legal events (each a decision), not one. On the step path it surfaces as a `kind: 'plan'` effect that **re-surfaces on every frontier** while the plan is in flight, its candidates recomputed from the live snapshot.
...
- [Hosts and executors](/docs/packages/agent/hosts): the executor functions the loop calls at each `text` / `decision` effect.
- [Checkpoints and the event log](/docs/packages/agent/checkpoints): the `AgentEventLogStore` protocol, forking, and time travel.
- [Human in the loop](/docs/packages/agent/human-in-the-loop): idle states and resuming from a persisted journal.
