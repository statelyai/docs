...
sourcePath: "docs/checkpoints.md"
sourceUrl: "https://github.com/statelyai/docs/blob/main/external-docs/agent/docs/checkpoints.md"
---
...
- `fork({ threadId, newThreadId, upToIndex })` copies a prefix onto a new thread. Rewind and diverge — LangGraph's time travel — is `fork` at an earlier index plus different events appended. See [examples/time-travel](https://github.com/statelyai/agent/blob/main/examples/time-travel/index.ts).
...
[`AgentSnapshotStore`](/docs/packages/agent/human-in-the-loop) remains useful as an **idle-point cache**: at a quiescent point (an idle state with no in-flight effects), persist the snapshot and resume from it plus the events appended since, instead of replaying from index 0. Compact only at quiescent points — a snapshot taken mid-flight cannot carry in-flight effect state; the log can.

## Related

- [The step path](/docs/packages/agent/steps): driving a machine one external input at a time.
- [Human in the loop](/docs/packages/agent/human-in-the-loop): idle states — the natural compaction points.
