...
- **Managed thin-loop helper.** A collapsed `await driveAgent(machine, { entries, executors, actorSources, store })` over the journal loop (see [the step path](/docs/packages/agent/steps)). Deferred because the loop is ~15 lines of host-owned code by design; a helper ships if alpha users keep hand-rolling the same wrapper.
