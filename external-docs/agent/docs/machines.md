...
**Emitted event schemas** type the progress events a machine emits with `enq.emit(...)`, received by hosts via [`runAgent`'s `on` handlers](/docs/packages/agent/hosts#observation-callbacks). Declare them under `emitted`:
...
The `agentSetup.createMachine` method is XState's `createMachine` with the agent's schemas and actors already bound. It registers the machine so the step helpers and [`runAgent`](/docs/packages/agent/hosts) resolve its schemas and actors without re-passing them.
...
See [examples/sql-agent/index.ts](https://github.com/statelyai/agent/blob/main/examples/sql-agent/index.ts).
