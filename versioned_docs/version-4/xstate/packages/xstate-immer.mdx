...
You don’t need to `import` anything from `immer`; it is a peer-dependency of `@xstate/immer`, so it must be installed.
...
:::warning .update(...) event creator

### Caution

The `.update(...)` event creator is pure; it only returns an assign action object, and doesn’t directly update `context`.
