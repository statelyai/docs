...
Actions are effects that are executed due to a [transition](LINK). Actions are fire-and-forget effects; that is, they are not awaited, and they don't return anything to the state machine.
