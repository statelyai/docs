...
Named options are preferred when using [typegen](/xstate/typescript/typegen) and also show up better when visualized with our [VS Code extension](/tools/xstate-vscode-extension). Named options show their name, whereas inline options show an “Inline” label.
