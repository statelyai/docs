...
- The action `type` describes the action. Actions with the same type have the same implementation.
...
Internally, when an event is raised, it is placed into an "internal event queue". These events are processed in insertion order (first-in first-out, or FIFO) after the current transition is finished. External events are not processed until all events in the internal event queue are processed.
...
If you only need to execute actions in response to events, you can create a [self-transition](transitions.mdx#self-transitions) that only has `actions: [ ... ]` defined. Fir example, a machine that only needs to assign to `context` in transitions may look like this:
