...
- `value` - the current state value, which is either a string representing:
  - a simple state, e.g. `'playing'`
  - or an object representing nested states, e.g. `{ paused: 'buffering' }`
