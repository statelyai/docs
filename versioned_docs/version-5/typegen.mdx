...
You can automatically generate intelligent typings for XState using our [VS Code extension](https://marketplace.visualstudio.com/items?itemName=statelyai.stately-vscode) or our [CLI](developer-tools#xstate-cli-command-line-interface).
...
You can use the [CLI](developer-tools.mdx#xstate-cli-command-line-interface) to regenerate them on CI (Continuous Integration), for instance, via a postinstall script:
