...
You can describe actor logic as a [state machine](#TODO). Actors created from state machine actor logic can:
...
Promise actor logic is logic that is described by an async process that resolves or rejects after some time. Actors created from promise logic ("promise actors") can:
...
Transition actor logic is logic that is described by a [transition function](#TODO), which is similar to a [reducer](#TODO). Transition functions are functions that take the current `state` and received `event` object as arguments, and return the next state. Actors created from transition logic ("transition actors") can:
...
Observable actor logic is logic that is described by an [observable stream of values](#TODO). Actors created from observable logic ("observable actors") can:
...
Event observable actor logic is logic that is described by an observable stream of [event objects](#TODO). Actors created from event observable logic ("event observable actors") can:
...
Callback actor logic is logic that is described by a callback function that receives a `sendBack` function and a `receive` function. Actors created from callback logic ("callback actors") can:
