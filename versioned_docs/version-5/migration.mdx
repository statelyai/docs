...
[Watch the Migrating to XState v5 beta video (1h4m)](https://www.youtube.com/watch?v=s-nqoIbIZac).
...
#### `predictableActionArguments` flag removed (now the default behavior)
...
#### Implementation functions now receive a single argument
...
#### Strict mode (removed) -> Use `*` (wildcard) transitions
...
#### `state.history` removed -> track with `actor.subscribe()` instead
...
#### `interpret()` now accepts a second options argument
...
#### `Use actor.getSnapshot()` to get an actor's current state
...
#### `actor.batch()` removed -> iterate with loop instead
...
#### `machineConfig.schema` (and `tsTypes`) -> machineConfig.types (and `typegen`)

The `machineConfig.schema` property is renamed to `machineConfig.types`, and `machineConfig.tsTypes` is now in `machineConfig.types.typegen`:
