...
### ~~`machine.withContext()`~~ Inject context with `input`
...
### ~~`predictableActionArguments`~~ Stable order by default
...
### ~~`in`~~ Use `guard: stateIn()` to validate state transitions
...
### ~~`state.history`~~ Track history with `actor.subscribe()`
...
### ~~`services`~~ Define an `actors` property on `options` object
...
### ~~`onTransition()`~~ Use `subscribe()` to listen for changes
...
### `interpret()` now accepts a second arg to restore state
...
### Use `actor.getSnapshot()` to get actor's current state
...
### ~~`actor.batch()`~~ Iterate over events with a loop instead
...
### ~~`machineConfig.schema`~~ is now `machineConfig.types`
