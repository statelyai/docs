...
Actors can persist their internal state and restore it later. **Persistence** refers to storing the state of an actor in persistent storage, such as [localStorage](#todo) or a database. **Restoration** refers to restoring the state of an actor from persistent storage.
...
// Persist events
localStorage.setItem('feedbackEvents', JSON.stringify(events));

// ...

// Restore state
const restoredEvents = JSON.parse(localStorage.getItem('feedbackEvents'));

// Replay events
const restoredFeedbackActor = interpret(feedbackMachine);

for (const event of restoredEvents) {
  restoredFeedbackActor.send(event);
}
```
