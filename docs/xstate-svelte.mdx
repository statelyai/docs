...
A function that creates an actor from the given finite state `machine` from [`@xstate/fsm`] and starts a service that runs for the lifetime of the component.
...
export const toggleService = createActor(toggleMachine).start();
```
