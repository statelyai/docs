...
You can restore an actor to a persisted state by passing the persisted state into the `state` option of the 2nd argument of `createActor(logic, { snapshot: restoredState })`:
...
An alternative to persisting state is **event sourcing**, which is a way of restoring the state of an actor by replaying the events that led to that state. Event sourcing can be more reliable than persisting state, because it is less prone to [incompatible state](#caveats) and it also allows you to replay actions.
