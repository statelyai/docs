...
**This feature is in beta!** Read the section below on known limitations to find out what we’re actively looking to improve.
...
You can automatically generate intelligent typings for XState using our [VS Code extension](https://marketplace.visualstudio.com/items?itemName=statelyai.stately-vscode) or our [CLI](/xstate/packages/xstate-cli).
...
Install the [CLI](/xstate/packages/xstate-cli) and run the `xstate typegen` command with the `--watch` flag.
...
You can use the [CLI](/xstate/packages/xstate-cli) to regenerate them on CI (Continuous Integration), for instance, via a postinstall script:
