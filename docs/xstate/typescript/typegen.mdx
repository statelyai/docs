...
Install the [CLI](/tools/developer-tools#xstate-cli-command-line-interface) and run the `xstate typegen` command with the `--watch` flag.
...
You can use the [CLI](/tools/developer-tools#xstate-cli-command-line-interface) to regenerate them on CI (Continuous Integration), for instance, via a postinstall script:
