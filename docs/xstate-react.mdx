...
Returns a [React Context object](https://beta.reactjs.org/learn/passing-data-deeply-with-context) that creates an actor from the `machine` and makes the actor available through React Context. There are helper methods for accessing state and the actor ref.
...
- `useActorRef()` - a React hook that returns the actor ref of the `machine` actor
