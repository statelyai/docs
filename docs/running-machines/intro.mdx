...
You can also use `state.value` to check your machine’s state, but we recommend using `state.matches` because of how it works with parent and child states. We’ll talk more about parent states later. [Jump directly to parent and child states](/states/advanced-transitions).
