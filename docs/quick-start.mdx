...
Machine logic can be used to create an actor. An [actor](actors.mdx) is a running process that can receive messages (events), send messages and change its behavior based on the messages it receives.
