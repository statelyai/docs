...
The `raise(...)` action raises (sends) an event to be received by the same machine immediately. Raised events are processed after the current transition is finished. This can be useful for triggering additional transitions within the same step based on the current event or state.
