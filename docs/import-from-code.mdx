...
Importing from code is helpful if you’ve already built machines while working with [XState](/xstate/intro.mdx), or have created a machine using our older [Stately Viz](https://stately.ai/viz) but haven’t yet tried the Stately Studio Editor.

Watch our [“Import from code” video on YouTube](https://www.youtube.com/watch?v=DAoIFaugDLo) (2m24s).

:::tip

Want to import machines from GitHub? Try our [import from GitHub](/import-from-github.mdx) options.

:::

Before import, your code should be formatted as a [`createMachine()` factory function](https://xstate.js.org/docs/guides/machines.html). The importer has basic validation in case your machine has basic errors, including reminding you if the `createMachine` definition is missing. 
...
You can now import multiple machines when importing code.

:::

If you have multiple `createMachine`s included in the code you insert in the text area, the Studio will import each machine as a new machine inside your project.
