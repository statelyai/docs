...
Actors can also be invoked on the _root_ of the machine, and they will be active for the lifetime of its parent machine actor:
...
- [Car vitals checks (SubFlow Repeat) workflow](https://github.com/statelyai/xstate/tree/next/examples/workflow-car-vitals)


## How are Actors Different From Actions?

Actions are “fire-and-forget”; as soon as their execution starts, the statechart running the actions forgets about them. If you specify an action as `async`, **the action won’t be awaited before moving to the next state**.

Invoked actors can do asynchronous work _and_ communicate with their parent machine. They they can send and receive events. Invoked machine actors can even invoke or spawn their own child actors.
...
- Event object `data` property is provided with actor’s error data
...
If a Promise rejects, the `onError` transition will be taken with a `{ type: 'xstate.error.actor.<id>' }` event. The error data is available on the event's `data` property:
        ...
        onError: {
          ...
          actions: assign({
            errorMessage: ({ context, event }) => {
              // event is:
              // { type: 'xstate.error.actor.<id>', data: 'No query specified' }
              return event.data;
            }
          })
        },
