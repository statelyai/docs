...
After creating your machine, you’ll need to create an API key to deploy it to Sky. You can do this using the **Run** button in the top right corner of the Studio. A screen will appear, prompting you to create an API key. Select that button.
...
Once you have generated the API key, you can deploy your machine to Sky as a running actor. Use the **Deploy new actor** button to start the deployment process. Once the actor is deployed, the page will display the name of the running actor and the URL you can use to interact with it. You’ll need to use that URL to communicate with the actor from the starter project.
...
At the project’s root, create a `.env` file to hold your API key. You need to set two variables in the `.env` file; paste your API key as the value for both these keys:
...
To fetch the config, we’ll use the [XState CLI tool](/docs/developer-tools.mdx#xstate-cli-command-line-interface). In our `package.json`, we already have a reference to the `sky` script we need to run. The script runs the command over all the files in the `src` repo to find configs associated with any initialized actors.
...
Once completed, you should see a second `skyConfig` argument with updated imports passed to the `actorFromStately` function. You should also see a new TypeScript file in your `src` directory, named after the actor in the Studio. In our case, it’s `trafficLightActor.sky.ts`.
