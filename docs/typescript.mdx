...
- Set [`exactOptionalPropertyTypes`](https://www.typescriptlang.org/tsconfig#exactOptionalPropertyTypes) to `true` in your `tsconfig.json` file. This will prevent assigning to `undefined` in partial assignments using the `assign(...)` action creator. (Recommended).
