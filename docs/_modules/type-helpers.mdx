...
// ---cut---
const myFunction = (
  event: EventFrom<typeof machine, "BAR">,
) => {
  console.log(event);
  //          ^?
};
...
const myFunction = (
  actor: InterpreterFrom<typeof machine>,
) => {
...
`ActorRefFrom` is especially useful when [spawning actors](/services/spawn) because it types the reference created by `spawn` when assigning to context.
