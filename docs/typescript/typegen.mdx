...
You can automatically generate intelligent typings for XState using our [VS Code extension](https://marketplace.visualstudio.com/items?itemName=statelyai.stately-vscode) or our [CLI](/reference/xstate-cli).

How to get started:

1. Download and install the [VS Code extension](https://marketplace.visualstudio.com/items?itemName=statelyai.stately-vscode) **or** install the [CLI](/reference/xstate-cli) and run the `xstate typegen` command with the `--watch` flag.
