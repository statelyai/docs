...
XState Store provides built-in undo/redo functionality through the `undoRedo` wrapper. This feature uses an _event-sourced approach_ rather than snapshot-based history, which is memory efficient (only stores events, not full state snapshots) and provides precise control over transaction grouping and event skipping while properly replaying side effects during undo/redo operations.

### Basic Usage

Import `undoRedo` from `@xstate/store/undo` and wrap your store configuration:
...
const store = createStore(
  undoRedo({
    ...
    on: {
      ...
      eatDonut: (context) => ({ ...context, donuts: context.donuts - 1 }),
    },
  }),
);

// Use the store normally
store.trigger.addDonut(); // donuts = 1
store.trigger.addDonut(); // donuts = 2

// Undo the last event
store.trigger.undo(); // donuts = 1

// Redo the undone event
store.trigger.redo(); // donuts = 2
```

### Transactions

You can group multiple events into transactions so they are undone/redone together:
...
// First transaction: adding donuts
store.send({ type: 'addDonut' });
store.send({ type: 'addDonut' });
// donuts = 2

// Second transaction: eating donuts
store.send({ type: 'eatDonut' });
store.send({ type: 'eatDonut' });
// donuts = 0

// Undo second transaction (both eaten)
store.trigger.undo();
// donuts = 2

// Undo first transaction (both added)
store.trigger.undo();
// donuts = 0
```
...
You can configure certain events to be skipped from the undo/redo history:

```ts
const store = createStore(
  undoRedo(
    {
      context: { donuts: 0, flavors: [] as string[] },
      on: {
        addDonut: (context) => ({ ...context, donuts: context.donuts + 1 }),
        logFlavor: (context, event: { flavor: string }, enq) => {
          // ... log the flavor
        },
      },
    },
    {
      // Skip flavor logging from undo/redo history
      skipEvent: (event, snapshot) => event.type === 'logFlavor',
    },
  ),
);

store.trigger.addDonut(); // donuts = 1
store.send({ type: 'logFlavor', flavor: 'chocolate' });
store.trigger.addDonut(); // donuts = 2
store.send({ type: 'logFlavor', flavor: 'glazed' });

// Undo skips flavor logs, only undoes addDonut events
store.trigger.undo(); // donuts = 1, flavors = [] (flavors lost because not replayed)
```

### Effects and Emitted Events

Emitted events are properly replayed during undo/redo operations:
...
const store = createStore(
  undoRedo({
    context: { donuts: 0 },
    emits: {
      donutChanged: (payload: { total: number }) => {},
    },
    on: {
      addDonut: (context, event, enqueue) => {
        enqueue.emit.donutChanged({ total: context.donuts + 1 });
        return { ...context, donuts: context.donuts + 1 };
      },
    },
  }),
);

store.on('donutChanged', (event) => {
  console.log('Donuts changed:', event.total);
});

store.trigger.addDonut(); // emits donutChanged(1)
store.trigger.undo(); // emits donutChanged(0)
store.trigger.redo(); // emits donutChanged(1)
...
The `undoRedo` wrapper accepts a second configuration object with these options:
...
const store = createStore(
  undoRedo(
    {
      context: { donuts: 0, batchId: null as string | null },
      on: {
        addDonut: (context) => ({ ...context, donuts: context.donuts + 1 }),
        setBatchId: (context, event: { id: string }) => ({
          ...context,
          batchId: event.id,
        }),
      },
    },
    {
      ...
      getTransactionId: (event, snapshot) => snapshot.context.batchId,

      // Skip events based on current state
      skipEvent: (event, snapshot) => snapshot.context.donuts >= 3,
    },
  ),
);
```
...
- **Type safety**: The undo/redo wrapper preserves all TypeScript types from your original store
- **Event-sourced**: History is maintained as events, not snapshots, making it memory efficient
