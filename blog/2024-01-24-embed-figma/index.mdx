...
image: /blog/2024-01-24-embed-figma/figma-dark.png
---

We already kicked off 2024 with several major updates to Stately such as inspecting live XState code, syncing with GitHub, and writing code for actions and actors in the Studio. Now we’re excited to announce that we’ve also launched an integration with Figma.

You can now attach frames in Figma to states in Stately.

<p>
  <ThemedImage
    alt="An example of Figma frames embedded in three Stately states for a music player."
    sources={{
...
It also ensures that everyone is always looking at the most up-to-date design, as changes made in Figma are instantly reflected in the embedded view. And with [GitHub Sync](https://stately.ai/blog/2024-01-09-introducing-bidirectional-github-sync) enabled, changes to the code can even automatically update how the Figma frames are structured in Stately.
...
[Watch a video](https://twitter.com/statelyai/status/1748407911649349966) of how it works from our recent Office Hours.

![How get get a frame link from Figma and add it to a Stately state](figmatostately.png)
...
- Stately supports linking to any type of Figma node that you can get,
  not just frames. But we recommend generally using frames due to how well they
