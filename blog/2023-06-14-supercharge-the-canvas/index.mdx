...
![Example of a flow](making-complexity-manageable.png)
...
![Adding an action from the Edit Menu](create-from-the-canvas.gif)
...
![Running effects from locations in the flow](effect-blocks-make-it.png)
...
![State name as header](streamlined-viz-1.png)
...
![Comparing states to transitions](streamlined-viz-2.png)
...
![Section borders around levels](streamlined-viz-3.png)
...
That’s why we made some canvas elements translucent by default, including state details and transition labels. It's usually better to avoid overlapping content, but now flows should be easier to follow when it happens.

![Translucency example](translucency.png)
...
And the Studio now supports several categories of [actions that are built into XState](/docs/xstate/actions/built-in-actions):
...
![Adding a color to a state](colors-2.gif)
