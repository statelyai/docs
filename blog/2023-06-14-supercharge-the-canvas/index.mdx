...
description: 'Today we are excited to release an update to the Stately Studio that takes a huge step towards making complexity more manageable. '
tags:
  [
    canvas,
    stately studio
...
Today we are excited to release an update to the Stately Studio that takes a huge step towards making complexity more manageable. 

- Bring color into your flows! 🌈
- Create more logic directly on the canvas 📝
...
- Use tags, `assign`, `raise`, and other XState actions in the Studio 💪<!--truncate-->
...
Being able to stay on the canvas lets interactions be more intuitive, lets you stay closer to your content, can help keep you focused, and even can give you more room to work. 

For example, we enhanced the Edit Menu to support adding actions, tags, and other data to states and transitions. And most items on the canvas can be edited directly.

- [ ]  gif adding a tag from quick actions and editing the name

## Effect blocks make it easy to say what happens when

The most important addition we’ve made is that effects (invoked actors, entry actions, and exit actions) are now interactive. They are represented as blocks on the canvas that can be placed inside of states and transitions. 

This will make it easy to define, understand, and modify what happens as users step through your flow. 
...
Speaking of upcoming features, if you follow [our live streams](https://youtube.com/statelyai/?source=blog), you’ve already seen a preview of writing code inside the Studio using the new effect blocks. But we decided to take this functionality even further before releasing, so stay tuned!
...
So we’ve turned over every aspect of how statecharts are represented on the canvas, trying to figure out how to balance showing various kinds of information in a way that is not overwhelming. 

One change is that the name of the state is now a header and most of the other data on it (tags, description, effects, and so on) are grouped together as part of a new detail section.
...
We think we’ve built something that will increasingly be able to support however you want to use the Stately Studio, smoothly growing from casual brainstorming sessions to business-critical production actor systems filled with multi-leveled machines.

## Translucency reduces ambiguities

Another part of the improved design is translucency. Because Stately handles the complex, nonlinear flows that are often required when building apps, it is not always possible to lay out your logic in a nice straight line. This means that arrows can go underneath of other items, making it difficult to read. Also, when working on a canvas sometimes it’s nice to casually toss items into overlapping piles as your work comes together. But statecharts can get pretty detailed! And this means sometimes states can cover each other up and it’s hard to even tell anything is underneath.
...
Try hiding the UI from the menu in the top left of the editor, or just press `⌘` + `.`  to toggle it instantly. We also support `⌘` + `\` for the same behavior if you prefer using that shortcut.

## Design with color

We have an enthusiastic community, and one request we’ve heard again and again was the need for color in flows. 
...
Finally, we are continuing to bring more of XState into the visual Studio. 
...
And the Studio now supports several categories of actions which are built into XState:
...
[Stately is free to try, so check it out](https://stately.ai). We’re sure you’ll find many ways it can help you out. And if there is anything you would love to see added, please [join our Discord community](https://discord.gg/xstate) or [reach out directly](mailto:support@stately.ai) and let us know!