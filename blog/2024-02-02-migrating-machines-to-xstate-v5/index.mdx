...
That still works but but it’s even easier to just cast the whole types object all at once.
...
In certain cases it can be helpful to convert some of our action and guard strings in the machine config to be parameterized objects. One of the major benefits is that this allows you to explicitly map `event` properties to a `params` object for your implementation functions so that they're automatically typed and there's no need for additional [type narrowing](https://www.typescriptlang.org/docs/handbook/2/narrowing.html).

In XState v4, we might have an event registered in the machine config like this:
...
Converting to objects is particularly helpful for transition actions or guards on those actions that will make use of `event` props so that these can be typed and used in the implementation functions. On the other hand, for `entry` and `exit` actions or actions and guards that only require `context` props, you can continue using the name as a `string` or even use an inline function for your action or guard. This is because `context` is always available to the machine and its types don’t vary.
...
Your `assign` actions will either be implemented in the machine config or passed to `setup()` and not [implemented externally](#5-provide-concrete-implementations) as they should have access to whatever they need between the typed `event` or the typed `params`.
...
const machine = setup({
  ...
  // predictableActionArguments are no longer needed
});
```
