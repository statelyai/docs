  ...
  on: {
    next: {
      ...
      actions: [
        {
          guard: {
            type: 'is this what I want',
            params: ({ event }) => ({ wanted: event.wanted }),
          },
          type: 'doThis',
...
When mapping `event` props, the action `params` you define can technically be any value, not just an object. But it is encouraged to use an object for consistency and to make it clear what the implementation functions will receive.
...
setup({
  guards: {
    'is this ready': (_, params: { list: string[] }) => {
      // Concrete implementation here
