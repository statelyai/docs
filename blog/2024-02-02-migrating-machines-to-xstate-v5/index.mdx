...
If you were previously using typegen in v4, you can now delete `tsTypes` as there is no more typegen in v5. Without typegen, typing events in actions and guards must be done manually using type narrowing in implementation functions. However, the following sections will show you how you can type a new `params` argument passed to those implementations and skip manual type narrowing.
