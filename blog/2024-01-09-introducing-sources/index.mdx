...
Would be given a default name of `#machine name.a.b.entry[1]`. These default names are not visible in the editor but will be used in the synced or exported code unless you give your action, actor, or guard a name. After first importing your machine, we recommend you give all your actions, actors, and guards a name. Naming is a one-time step, and you can always rename them later. Giving your sources distinct names also means that multiple actors, actions, or guards can reference them, which can be really useful when combined with parameters.
